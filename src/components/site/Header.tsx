    ...
    >
      <nav className="mx-auto flex h-[60px] max-w-[1200px] items-center justify-between px-6 md:px-10">
        ...
        <div className="hidden items-center gap-0.5 lg:flex">
          {/* Primary nav items */}
          {PRIMARY_NAV.filter((i) => !i.secondary).map((item) => {
            if (item.kind === "link") {
              return (
                <Link
                  key={item.label}
                  to={item.to}
                  className="rounded-md px-3 py-2 text-[14.5px] font-medium tracking-[-0.02em] text-white/55 transition-colors hover:text-white focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-primary/50"
            ...
            return (
              ...
              >
                <button
                  type="button"
                  className={cn(
                    "flex items-center gap-1 rounded-md px-3 py-2 text-[14.5px] font-medium tracking-[-0.02em] text-white/55 transition-colors hover:text-white focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-primary/50",
                    isOpen && "text-white",
                  )}
          ...
          {/* Divider */}
          <span className="mx-1.5 h-4 w-px shrink-0 bg-white/10" aria-hidden />

          {/* Secondary nav cluster — lighter weight */}
          {PRIMARY_NAV.filter((i) => i.secondary).map((item) => {
            if (item.kind === "link") {
              return (
                <Link
                  key={item.label}
                  to={item.to}
                  className="rounded-md px-2.5 py-2 text-[13px] font-medium tracking-[-0.01em] text-white/35 transition-colors hover:text-white/70 focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-primary/50"
                  activeProps={{ className: "text-white/70" }}
                >
                  {item.label}
                </Link>
              );
          ...
          <div className="mx-auto max-w-[1200px] px-6 py-4 md:px-10">
            <nav className="flex flex-col">
              ...
              <div className="mt-2 border-t border-white/5 pt-2">
                <p className="px-3 pb-1.5 pt-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-white/30">
                  Resources
                </p>
                <div className="flex flex-wrap gap-1 px-3">
                  {PRIMARY_NAV.filter((i) => i.secondary && i.kind === "link").map((item) => (
                    <Link
                      key={item.label}
                      to={(item as { to: string }).to}
                      className="rounded-md px-3 py-2 text-[14px] text-white/60 hover:bg-white/5 hover:text-white"
                    >
                      {item.label}
                    </Link>
                  ))}
